...
export const useSls0432Store = defineStore('sls-0432', () => {
    ...
    const cols = computed(() => {
        const obj = {};
        colNames.forEach(col => { obj[col.toLowerCase().trim().replace(/[' '#%]/g,'')] = reportHeader.value.indexOf(col) });
        return obj;
    });
    ...
    const deals = computed(() => {
        const idCol = cols.value["salesperson"];
        const unitCountCol = cols.value["units"];
        const deals = report.value.filter(row => Number(row[idCol]) === currentSalesman.value && row[unitCountCol] > 0);
        return deals.map(deal => {
            const id = deal[cols.value["reference"]];
            const date = deal[cols.value["date"]];
            const vehicle = new Vehicle(deal[cols.value["stock"]], deal[cols.value["description"]], deal[cols.value["saletype"]]);
            const unitCount = deal[cols.value["units"]];
            const commission = { gross: deal[cols.value["commblgross"]], amount: deal[cols.value["commissionamount"]] };
    ...
    const allDeals = computed(() => {
        ...
        report.value.forEach(row => {
            ...
            if(row[unitCountCol]) {
                ...
                const vehicle = new Vehicle(row[cols.value["stock"]], row[cols.value["description"]], row[cols.value["saletype"]]);
                const unitCount = row[cols.value["units"]];
                const commission = { gross: row[cols.value["commblgross"]], amount: row[cols.value["commissionamount"]] };
                obj[id] = [];
                obj[id].push({ dealId, date, vehicle, unitCount, commission });
            }
        });
        return obj;
    });

    return { reportHeader, report, cols, salesmen, currentSalesman, deals, allDeals };
});